...
# # #     # --------------------------------------------------
# # #     # Job processing (FINAL, GUARDED)
# # #     # --------------------------------------------------
# # #     async def process_job(self, job: Dict[str, Any]):
# # #         job_id = job.get("id")

# # #         # 🔒 CRITICAL GUARD — DO NOT PROCESS BEFORE UPLOAD FINISHES
# # #         if job.get("status") != "uploaded":
# # #             logger.info(
# # #                 f"⏳ Skipping job {job_id} "
# # #                 f"(status={job.get('status')}) — upload not finished"
# # #             )

# # #             # move back to pending so it can be retried later
# # #             self.redis.lrem(QUEUES["processing"], 0, json.dumps(job))
# # #             self.redis.lpush(QUEUES["pending"], json.dumps(job))
# # #             return
...
# # #             logger.info(
# # #                 f"🧠 Router selected "
# # #                 f"{job['router_decision']['provider']} for job {job_id}"
# # #             )
...
# #     # --------------------------------------------------
# #     # Supabase status check (SOURCE OF TRUTH)
# #     # --------------------------------------------------
# #     async def fetch_db_status(self, job_id: str) -> Optional[str]:
# #         async with aiohttp.ClientSession() as session:
# #             async with session.get(
# #                 f"{SUPABASE_URL}/rest/v1/video_jobs"
# #                 f"?id=eq.{job_id}&select=status",
# #                 headers={
...
# #         # 🔒 CHECK SUPABASE (NOT REDIS)
# #         db_status = await self.fetch_db_status(job_id)

# #         if db_status != "uploaded":
# #             logger.info(
# #                 f"⏳ Skipping job {job_id} "
# #                 f"(db_status={db_status}) — upload not finished"
# #             )
...
# #                 if resp.status == 200:
# #                     return await resp.json()
# #                 return {"success": False, "error": f"Router HTTP {resp.status}"}
...
# #                 if resp.status == 200:
# #                     return await resp.json()
# #                 return {"success": False, "error": f"HF Farm HTTP {resp.status}"}
...
# if not REDIS_URL or not CLOUDFLARE_ROUTER_URL or not HF_FARM_URL:
#     raise RuntimeError("One or more required environment variables are missing")
...
#     "pending": "video_jobs:pending",
#     "processing": "video_jobs:processing",
#     "completed": "video_jobs:completed",
#     "dead_letter": "video_jobs:dead_letter",
#     "retry_delayed": "video_jobs:retry:delayed",
# }
...
#     # --------------------------------------------------
#     # Job processing (FINAL GUARDED LOGIC)
#     # --------------------------------------------------
#     async def process_job(self, job: Dict[str, Any]):
#         job_id = job.get("id")
#         db_status = job.get("status")

#         # ✅ HARD GUARD — only process AFTER upload is finished
#         if db_status not in ("uploaded", "routed"):
#             logger.info(
#                 f"⏳ Skipping job {job_id} "
#                 f"(db_status={db_status}) — upload not finished"
#             )
...
#             # 3️⃣ Finalize
#             # ----------------------------
#             self.redis.lpush(QUEUES["completed"], json.dumps(job))
#             self.redis.lrem(QUEUES["processing"], 0, json.dumps(job))
...
class QueueWorker:
    ...
    async def start(self):
        logger.info("🚀 Queue Worker started")
        logger.info(f"Router Worker: {CLOUDFLARE_ROUTER_URL}")
        logger.info(f"HF Farm: {HF_FARM_URL}")
    ...
    async def process_job(self, job: Dict[str, Any]):
        ...
        # 🛑 CRITICAL GUARD — DO NOT PROCESS EARLY
        if db_status != "uploaded":
            logger.info(
                f"⏸️ Ignoring job {job_id} "
                f"(db_status={db_status}) — waiting for upload"
            )

            # Remove from processing and STOP
            self.redis.lrem(QUEUES["processing"], 0, json.dumps(job))
            return
